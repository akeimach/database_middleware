...
public class DBView {
	...
	public static void setDBcolumns() {
		...
		fields[0] = "id_0";
		for (int col = 1; col < numDummyCols; col++) {
			fields[Parser.tableSize + col] = "version_" + (Parser.tableSize + col);
		}
		int title_id = 0;
		//set the rest of the columns to the standard titles
		for (int i = 1; i <= Parser.tableSize; i++) {
			if (Parser.titlesIncluded) {
				fields[i] = Parser.titleRow[title_id];
			} else {
				fields[i] = "col_" + i;
			}
			title_id++;
		...
		types[0] = "INT";
		for (int col = 1; col < numDummyCols; col++) {
			types[Parser.tableSize + col] = "INT NULL";
		}
		int type_id = 0;
		for (int i = 1; i <= Parser.tableSize; i++) {
	...
	public static void updateDBstrings() {
		...
		fields[0] = "id_0";
		for (int col = 1; col < numDummyCols; col++) {
			fields[UserView.tableSize + col] = "version_" + (UserView.tableSize + col);
		}
		int title_id = 0;
		//set the rest of the columns to the standard titles
		for (int i = 1; i <= Parser.tableSize; i++) {
			if (Parser.titlesIncluded) {
				fields[i] = UserView.fields[title_id];
			} else {
				fields[i] = "col_" + i;
			}
			title_id++;
		...
		types[0] = "INT";
		for (int col = 1; col < numDummyCols; col++) {
			types[UserView.tableSize + col] = "INT NULL";
		}
		int type_id = 0;
		for (int i = 1; i <= Parser.tableSize; i++) {
			types[i] = Parser.types[type_id];
