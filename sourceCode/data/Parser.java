...
public class Parser {
	...
	public static void patternMatcher(String value, int i) {
		...
		if (Pattern.matches(CHAR, value)) {
			if (sizes[i] < 10) {
				types[i] = "CHAR(" + sizes[i] + 5 + ")";
			} else {
				types[i] = "VARCHAR(100)";
			}
		} else if (Pattern.matches(FLOAT, value)) {
			types[i] = "FLOAT";
		} else if (Pattern.matches(INT, value)) {
			types[i] = "BIGINT";
		} //for testing
		else if (Pattern.matches(DDMMYYYY, value) || Pattern.matches(MMDDYYYY, value) || Pattern.matches(MMDDYY, value)) {
			types[i] = "DATE";
		} else if (Pattern.matches(HOUR24, value) || Pattern.matches(HOUR12, value)) {
			types[i] = "TIME";
		}
		else if (Pattern.matches(YYYYMMDDHHMMSS, value)) {
			types[i] = "TIMESTAMP";
		} else {
			types[i] = "VARCHAR(100)";
		}
